...
const cardData = [
  {
    color: '#060010',
    title: 'Tracking',
    description: 'Monitor and analyze user behavior to optimize your app.',
    label: 'Performance',
    src: '/images/1.png',
    srcVisible: false,
  },
  ...
  },
  {
    color: '#060010',
    title: '',
    description: 'Deliver experiences that keep your users happy and engaged.',
...
const MagicBento = ({ textAutoHide = true, enableStars = true, enableSpotlight = true, enableBorderGlow = true, disableAnimations = false, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, particleCount = DEFAULT_PARTICLE_COUNT, enableTilt = false, glowColor = DEFAULT_GLOW_COLOR, clickEffect = true, enableMagnetism = true }) => {
  ...
  function showImage(card, srcVisible) {
    if (!srcVisible) return <></>;
    else {
      return (
        <div className="magic-bento-card__image">
          <img className="magic-bento-card__images mx-auto my-4" src={card.src} alt={card.src} />
        </div>
      );
    }
  }
          ...
          if (enableStars) {
            return (
              <ParticleCard key={index} {...cardProps} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
                ...
                {showImage(card, card.srcVisible)}
